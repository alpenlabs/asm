...
//! recursed over.
//!
//! Building the expected transition needs this node's own storage, so it lives
//! on [`InputBuilder`](super::input::InputBuilder) alongside the other reads
//! of it, and [`ProofVerifier::verify`] stays sync and pure. That split is
//! also what the two failures mean: failing to read our own state says nothing
//! about the source that served the receipt, while a receipt that does not
//! match it says everything.
...
/// Why a receipt is not an acceptable proof of the
/// [`ProofId`](strata_asm_proof_types::ProofId) it is filed under.
#[derive(Debug, Error)]
...
pub(crate) struct EndpointMismatch {
    ...
    pub(crate) expected: StateRefAttestation,
    /// The endpoint the receipt actually commits to.
    pub(crate) actual: StateRefAttestation,
}
...
/// [`InputBuilder::expected_attestation`](super::input::InputBuilder::expected_attestation)
/// and compared whole by [`ProofVerifier::verify`], which also takes the proof
/// kind from it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum ExpectedAttestation {
...
/// Checks proof receipts against the predicate keys the orchestrator proves
/// under.
///
...
/// itself.
///
/// Borrows the key pair the input builder already holds rather than owning
/// one, so it is built for the duration of one orchestration step. No proving
/// host is needed: a predicate key carries everything the check requires,
/// which is why the guest can run the same one.
#[derive(Debug)]
pub(crate) struct ProofVerifier<'a> {
    asm: &'a PredicateKey,
    moho: &'a PredicateKey,
}

impl<'a> ProofVerifier<'a> {
    /// Creates a verifier over the `(asm, moho)` predicate key pair.
    pub(crate) fn new(asm: &'a PredicateKey, moho: &'a PredicateKey) -> Self {
        Self { asm, moho }
    }
...
pub(crate) fn state_reference(block: &L1BlockCommitment) -> StateReference {
...
mod tests {
    ...
    /// [`resolve_predicate`](super::super::backend).
    ...
    /// What the input builder builds for an ASM step proof of `TARGET`: out of
    /// its parent, into the block.
    ...
    fn verifier<'a>(asm: &'a PredicateKey, moho: &'a PredicateKey) -> ProofVerifier<'a> {
        ProofVerifier::new(asm, moho)
    }

    /// The `(asm, moho)` predicates the orchestrator proves under.
    ...
    /// while the orchestrator derives it from the block id in the
    /// [`ProofId`](strata_asm_proof_types::ProofId). Both have to land on the
    /// same 32 bytes or every comparison would fail.
    #[test]
