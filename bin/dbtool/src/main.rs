...
//!
//! Covers the `asm` domain (ASM DB: anchor state, aux data, manifests, and
//! the manifest-hash MMR) and the `moho` domain (Moho DB: state snapshots and
//! the export-entry index). The proof-DB resources (`proof …`) land in a
