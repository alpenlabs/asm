...
class AsmDbtoolMappingHealTest(flexitest.Test):
    """Deleting a proof leaves its block stuck; deleting the mapping frees it.
    ...
    Only Moho recursion stops. ASM step proofs have no proof prerequisites, so
    they keep being produced throughout, which is what this test uses to show
    the prover is alive rather than merely idle.
    """
    ...
    def main(self, ctx: flexitest.RunContext):
        ...
        # ---- It stays stuck across the restart ----
        #
        ...
        for _ in range(STUCK_OBSERVATION_TICKS):
            time.sleep(1)
