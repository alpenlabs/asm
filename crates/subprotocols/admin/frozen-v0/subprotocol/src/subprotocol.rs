...
use strata_asm_proto_admin_txs_v0::{constants::ADMINISTRATION_SUBPROTOCOL_ID, parser::parse_tx};
...
impl Subprotocol for AdministrationSubprotocol {
    ...
    // The released layout. The current one appends
    // `ol_transition_pending`, which changes the encoding, so it declares 1.
