...
use strata_asm_proto_admin_txs_v0::{
...
use strata_asm_proto_checkpoint_msgs_v0::CheckpointIncomingMsg;
...
fn relay_checkpoint_sequencer_update(relayer: &mut impl MsgRelayer, new_key: Buf32) {
    // DEVIATION: `PredicateKey::new` became `try_new`. The released `new`
    // ended in `.expect("valid length")`, so this panics on exactly the same
    // input and the behaviour is unchanged.
    let msg = CheckpointIncomingMsg::UpdateSequencerKey(
        PredicateKey::try_new(PredicateTypeId::Bip340Schnorr, new_key.0.to_vec())
            .expect("valid length"),
    );
