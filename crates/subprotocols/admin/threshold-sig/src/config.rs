//! Signer rosters for the administration threshold scheme.
...
///
/// A signer identifies itself by a `u8` index into the roster, so no more than 256 signers
/// are addressable.
...
/// `NonZero<u8>` so that a roster which no one can satisfy cannot be constructed.
///
...
impl ThresholdConfig {
    ...
    /// and [`ThresholdSignatureError::InvalidThreshold`] if `threshold` exceeds the roster
    /// size.
    pub fn try_new(
        keys: Vec<CompressedPublicKey>,
        threshold: NonZero<u8>,
...
impl SszContainer for ThresholdConfig {
    ...
    fn to_container(&self) -> Self::Container {
        ...
        // rejects a roster larger than `MAX_SIGNERS`.
        let keys = self
            .keys
            .iter()
            ...
            .collect::<Vec<_>>()
            .try_into()
            .expect("roster is within MAX_SIGNERS");
    ...
    fn from_container(container: Self::Container) -> Result<Self, DecodeError> {
        ...
        // Re-applies the roster invariants, so a decoded config is indistinguishable from a
        // constructed one.
...
mod arbitrary_impls {
    ...
    impl<'a> Arbitrary<'a> for ThresholdConfig {
        fn arbitrary(u: &mut Unstructured<'a>) -> Result<Self> {
            // Small rosters keep generated values cheap and well inside MAX_SIGNERS.
...
mod tests {
    ...
    #[test]
    fn try_new_rejects_threshold_above_roster_size() {
    ...
    fn deserialize_rejects_more_than_max_signers() {
        // The validating `Deserialize` routes through `try_new`, so an oversized roster is
    ...
    fn arb_threshold_config() -> impl Strategy<Value = ThresholdConfig> {
        (1usize..=8)
            ...
            .prop_map(|(n, threshold)| {
                ...
                ThresholdConfig::try_new(keys, nonzero(threshold)).expect("roster is valid")
            })
    }
