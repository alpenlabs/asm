...
/// Hashing by the compressed encoding lets signer keys be deduplicated with a set, which
/// [`ThresholdConfig`](crate::ThresholdConfig) relies on. `PublicKey` itself is not `Hash`.
