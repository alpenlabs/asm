...
impl AssignmentEntry {
    ...
    pub fn net_amount(&self) -> BitcoinAmount {
        // Saturating at zero, as released. The current amount type has no
        // saturating_sub of its own, so the arithmetic is done on satoshis and
        // rewrapped through the infallible From<Amount>.
        BitcoinAmount::from(Amount::from_sat(
            self.withdrawal_output
                .amt()
                .to_sat()
                .saturating_sub(self.operator_fee.to_sat()),
        ))
    }
