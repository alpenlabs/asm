...
use strata_asm_proto_bridge_txs::{BRIDGE_SUBPROTOCOL_ID, parser::parse_tx};
...
// The id a released bridge section is stored under, and the id its export
...
const _: () = assert!(BRIDGE_SUBPROTOCOL_ID == 2);

impl Subprotocol for BridgeV1Subproto {
    const ID: SubprotocolId = BRIDGE_SUBPROTOCOL_ID;

    // The released layout, which is also the current one: every encoded
    // struct in the bridge state was verified identical against the release
    // tag (only doc comments differ), so this codec version does not advance.
    // The two specs' schemas still differ overall, because checkpoint's does.
    ...
    fn process_txs(
        ...
        // DEVIATION: the released code read a public field and went through
        // `into_native()` to reach the little-endian bytes. The field is now
        // private with an accessor, and `BtcWork` exposes `to_le_bytes()`
        // directly. The same 32 little-endian bytes of the same value reach the
        // log either way, which matters because this payload feeds the manifest
        // and so the state root.
