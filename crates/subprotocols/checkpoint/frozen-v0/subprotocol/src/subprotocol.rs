...
// DEVIATION: the subprotocol init configs moved out of the params crate into
// each subprotocol's own types crate (commit bdfe800). Import path only.
...
impl Subprotocol for CheckpointSubprotocolV0 {
    ...
    // The released layout. `7e0b873` inserted `pending_transition`
    // mid-container, which shifts every field after it, so the current
    // implementation declares 1.
