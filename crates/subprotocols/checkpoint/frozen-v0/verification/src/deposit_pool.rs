...
impl DepositPool {
    ...
    pub(crate) fn new_empty() -> Self {
        Self {
            denomination: BitcoinAmount::from(Amount::ZERO),
            count: 0,
        }
    }
    ...
    pub(crate) fn total(&self) -> BitcoinAmount {
        BitcoinAmount::from(Amount::from_sat(
            self.denomination.to_sat() * self.count as u64,
        ))
    }
    ...
    pub(crate) fn is_empty(&self) -> bool {
        self.count == 0 && self.denomination == BitcoinAmount::from(Amount::ZERO)
    }
    ...
    pub(crate) fn verify_withdrawals(
        ...
        if self.count == 0 {
            return Err(InvalidCheckpointPayload::InsufficientFunds {
                available: BitcoinAmount::from(Amount::ZERO),
                required: BitcoinAmount::from(Amount::from_sat(
                    intents.iter().map(|w| w.amt().to_sat()).sum(),
                )),
            });
        }
...
mod tests {
    ...
    fn withdrawal(sats: u64) -> WithdrawalIntent {
        WithdrawalIntent::new(
            dummy_descriptor(),
            BitcoinAmount::from(Amount::from_sat(sats)),
    ...
    fn empty_pool_total_is_zero() {
        ...
        assert_eq!(pool.total(), BitcoinAmount::from(Amount::ZERO));
    }

    #[test]
    fn record_sets_denomination_and_counts() {
        let mut pool = DepositPool::default();
        let denom = BitcoinAmount::from(Amount::from_sat(500_000_000));
        ...
        assert_eq!(
            pool.total(),
            BitcoinAmount::from(Amount::from_sat(1_500_000_000))
        );
    }
    ...
    fn deduct_exact_denomination_match() {
        ...
        assert_eq!(
            pool.total(),
            BitcoinAmount::from(Amount::from_sat(500_000_000))
        );
    }
    ...
    fn denomination_mismatch_fails() {
        let mut pool = DepositPool::default();
        pool.record(BitcoinAmount::from(Amount::from_sat(1_000_000_000)));
        ...
        assert!(matches!(
            ...
            if expected == BitcoinAmount::from(Amount::from_sat(1_000_000_000))
                && actual == BitcoinAmount::from(Amount::from_sat(500_000_000))
        ));

        assert_eq!(
            pool.total(),
            BitcoinAmount::from(Amount::from_sat(1_000_000_000))
        );
    }
    ...
    fn insufficient_count_fails() {
        ...
        assert!(matches!(
            ...
            if available == BitcoinAmount::from(Amount::from_sat(500_000_000))
                && required == BitcoinAmount::from(Amount::from_sat(1_000_000_000))
        ));
    ...
    fn withdrawal_against_empty_pool_fails() {
        ...
        assert!(matches!(
            err,
            InvalidCheckpointPayload::InsufficientFunds { available, required }
            if available == BitcoinAmount::from(Amount::ZERO)
                && required == BitcoinAmount::from(Amount::from_sat(500_000_000))
        ));
    }
    ...
    fn batch_withdrawal_same_denomination() {
        let mut pool = DepositPool::default();
        let denom = BitcoinAmount::from(Amount::from_sat(100_000_000));
        ...
        assert_eq!(
            pool.total(),
            BitcoinAmount::from(Amount::from_sat(200_000_000))
        );
    }
    ...
    fn multi_denomination_intent_exceeding_pool_fails() {
        ...
        assert!(matches!(
            err,
            InvalidCheckpointPayload::InsufficientFunds { available, required }
            if available == BitcoinAmount::from(Amount::from_sat(200_000_000))
                && required == BitcoinAmount::from(Amount::from_sat(300_000_000))
        ));
    }
    ...
    fn non_multiple_intent_fails() {
        ...
        assert!(matches!(
            err,
            InvalidCheckpointPayload::DenominationMismatch { expected, actual }
            if expected == denom && actual == BitcoinAmount::from(Amount::from_sat(150_000_000))
        ));
    }
    ...
    fn zero_amount_intent_fails() {
        ...
        assert!(matches!(
            err,
            InvalidCheckpointPayload::DenominationMismatch { expected, actual }
            if expected == denom && actual == BitcoinAmount::from(Amount::ZERO)
        ));
    }
    ...
    fn empty_intents_succeed_with_deposits() {
        let mut pool = DepositPool::default();
        pool.record(BitcoinAmount::from(Amount::from_sat(500_000_000)));
