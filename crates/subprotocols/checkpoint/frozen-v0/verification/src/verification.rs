...
pub(crate) fn extract_withdrawal_intents(
        ...
        .filter(|l| l.account_serial() == BRIDGE_GATEWAY_ACCT_SERIAL)
    {
        ...
        // The released rules built this amount unchecked, so it is wrapped through
        // the infallible `From<Amount>` rather than the current type's checked
        // `TryFrom<u64>`. The value comes from an OL log, not a Bitcoin output, so
        // nothing bounds it by consensus; rejecting it here instead would change
        // which checkpoints verify.
        let amt = BitcoinAmount::from(Amount::from_sat(withdrawal_data.amt()));
        let withdraw_output = WithdrawalIntent::new(destination, amt, selected_operator);
...
mod tests {
    ...
    fn test_extract_dispatches_on_log_type() {
        ...
        assert_eq!(
            outputs[0].amt(),
            BitcoinAmount::from(Amount::from_sat(withdrawal.amt()))
        );
    }
