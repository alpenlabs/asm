...
//! genesis L1 view, the predicate the chain starts under, and the typed
//! subprotocol configurations needed to construct the initial ASM state. The
//! per-subprotocol configurations are defined in each subprotocol's own types
//! crate and only aggregated (and re-exported) here.
//!
//! After genesis, this configuration is not consulted for STF or subprotocol
//! dispatch. The runner retains it for the control RPC; later revisions obtain
//! authenticated upgrade parameters through the upgrade mechanism.
