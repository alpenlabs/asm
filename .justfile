...
prover-eval: prover-clean
  cd {{prover_perf_eval_dir}} && RUST_LOG=info ZKVM_MOCK=1 ZKVM_PROFILING=1 BUILD_ELF=1 cargo run --release -- --programs {{prover_programs}}

# Generate proofs
prover-proof:
  cd {{prover_perf_eval_dir}} && RUST_LOG=info BUILD_ELF=1 cargo run --release -- --generate-proof --programs {{prover_programs}}
